<!-- Header Section -->
<div align="center">
  <!-- LeetCode Logo -->
  <img src="https://upload.wikimedia.org/wikipedia/commons/1/19/LeetCode_logo_black.png" alt="LeetCode Logo" width="140" />
  <img src="https://github.com/shriram-02/shriram-02/blob/main/LeetCode%20Daily%20coin.gif?raw=true" width="140" />

  <h1>🏆 LeetCode Problem of the Day Solutions</h1>
</div>

<div align="center">

## 🤝 Connect with Me

<p>
  <a href="https://github.com/shriram-02">
    <img src="https://skillicons.dev/icons?i=github" height="32" width="32" />
  </a>

  <a href="https://www.linkedin.com/in/shriram-lahane-12b692385/">
    <img src="https://skillicons.dev/icons?i=linkedin" height="32" width="32" />
  </a>

  <a href="https://leetcode.com/u/shriram_lahane/">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/leet-code.svg" height="32" width="32" />
  </a>

  <a href="https://auth.geeksforgeeks.org/user/shriram01">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/geeks-for-geeks.svg" height="32" width="32" />
  </a>

  <a href="https://www.hackerrank.com/profile/lahaneshriram2">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/hackerrank.svg" height="32" width="32" />
  </a>

  <a href="https://kaggle.com/shriramlahane">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/kaggle.svg" height="32" width="32" />
  </a>

  <a href="https://instagram.com/pvt.shree_01">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/instagram.svg" height="32" width="32" />
  </a>

  <a href="https://discord.com/users/shriram_79991">
    <img src="https://skillicons.dev/icons?i=discord" height="32" width="32" />
  </a>
</p>

</div>

<div align="center">
  <a href="https://leetcode.com/u/shriram_lahane/">
    <img src="https://img.shields.io/badge/View%20LeetCode%20Profile-FFA116?style=for-the-badge&logo=leetcode&logoColor=white" alt="View LeetCode Profile" />
  </a>
</div>

<hr />

# 📌 Overview

The **LeetCode Problem of the Day Solutions** repository is a comprehensive collection of **daily LeetCode challenge solutions** solved consistently to strengthen **Data Structures and Algorithms (DSA)** skills and enhance **technical interview preparation**.

This repository serves as a structured resource for students, software engineers, and coding enthusiasts who wish to follow a disciplined problem-solving routine. Every solution is carefully written with an emphasis on **clean code**, **optimized approaches**, and **clear understanding of algorithms**.

The repository includes solutions from various difficulty levels ranging from **Easy**, **Medium**, to **Hard**, covering a wide spectrum of topics such as:

- Arrays
- Strings
- Linked Lists
- Trees & Binary Trees
- Binary Search
- Dynamic Programming
- Graphs
- Greedy Algorithms
- Backtracking
- Sliding Window
- Stack & Queue
- Heap / Priority Queue
- Trie
- Union Find (DSU)
- Bit Manipulation
- Math
- Recursion
- Hashing
- Prefix Sum
- Segment Tree
- And many more...

Each solution aims to provide:

- ✅ Optimized Code
- ✅ Clean Implementation
- ✅ Interview-Friendly Logic
- ✅ Well-Structured Repository
- ✅ Daily Consistency

---
